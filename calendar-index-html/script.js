...
function createCalendar() { // カレンダーを作る関数
  ...
  for (let weekIndex = 0; weekIndex < weekCount; weekIndex++) { // 表示する週数だけ繰り返す
    ...
    weekTitle.textContent = `${formatDate(weekStart)}〜${formatDate(weekEnd)}`; // 何週目を出さず日付範囲だけ表示する
    ...
    for (let period = 1; period <= 4; period++) { // 1限から4限まで繰り返す
      ...
      for (let dayIndex = 0; dayIndex < 7; dayIndex++) { // 日曜から土曜まで繰り返す
        ...
        cell.addEventListener("click", () => { // ボックスがクリックされたときの処理を設定する
...
function changeAbsenceState(dayIndex, period) { // 欠席色を切り替える関数
  ...
  const currentIndex = states.indexOf(currentState); // 現在の状態が配列の何番目か調べる
